...
export class EducationComponent
  ...
  eduHistory: Observable <EducationHistory[]>;
  ...
  constructor(
    ...
    public thisService: EducationHistoryService
  ) {}

  ngOnInit(): void {
    let userId =localStorage.getItem('userId');
    this.endpoint=`getUserEducation?adminId=${userId}&userId=${this.professionalProfileId}`
    this.searchForm();
  ...
  */
  ...
  delete(id) {
    ...
    let userId =localStorage.getItem('userId');
      this.endpoint=`deleteEducation?educationId=${id}&adminId=${userId}`
  ...
  edit(id: number): void {
    const modalRef = this.modalService.open(EditEducationComponent, { size: 'lg' });
    modalRef.componentInstance.id = id;
