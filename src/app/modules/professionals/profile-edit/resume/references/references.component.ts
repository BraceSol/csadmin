...
ISearchView {
  ...
  constructor(
    ...
    public thisService: ReferenceService
  ) { }
  ngOnInit(): void {
    let userId =localStorage.getItem('userId');
    this.endpoint=`getUserReferences?userId=${this.professionalProfileId}&adminId=${userId}`
    this.searchForm();
  ...
  */
  ...
  delete(id) {
    let userId =localStorage.getItem('userId');
      this.endpoint=`deleteReference?referenceId=${id}&adminId=${userId}`
  ...
  edit(id) {
    const modalRef = this.modalService.open(EditReferenceComponent, { size: 'lg' });
    modalRef.componentInstance.id = id;
