...
@Component({
  selector: 'app-employerqueues',
  templateUrl: './queues.component.html'
})
export class EmployerQueuesComponent
  implements
    OnInit,
    OnDestroy,
    IDeleteAction,
  ...
  let userId =localStorage.getItem('userId');
    this.endpoint=`getQueus?adminId=${userId}&isSystem=1`
    this.searchForm();
...
// this.router.navigate(['/employers/listing'])
  ...
  edit(id: number): void {
    const modalRef = this.modalService.open(EditQueueComponent, { size: 'lg' });
    modalRef.componentInstance.id = id;
