...
@Component({
  selector: 'app-employeraudits',
  templateUrl: './audits.component.html'
})
export class EmployerAuditsComponent
  implements
    OnInit,
    OnDestroy,
    IDeleteAction,
  ...
  constructor(
    ...
    public thisService: EmployerAuditsService
  ) {}

  ngOnInit(): void {

    let userId =localStorage.getItem('userId');
    this.endpoint=`getAudits?adminId=${userId}&employerId=${this.employerId}`
    this.searchForm();
  ...
  */
  ...
  let userId =localStorage.getItem('userId');
    this.endpoint=`deleteAudit?adminId=${userId}&auditId=${id}`//&employerId=${this.employerId}
...
// this.router.navigate(['/employers/listing'])
  ...
  edit(id: number): void {
    const modalRef = this.modalService.open(EditAuditComponent, { size: 'lg' });
    modalRef.componentInstance.id = id;
