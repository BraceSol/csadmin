...
export class ComplianceReportsComponent
  ...
  constructor(
    ...
    public thisService: ComplianceReportsService
  ) {}

  ngOnInit(): void {

    let userId =localStorage.getItem('userId');
    this.endpoint=`getComplianceReports?adminId=${userId}`
    this.searchForm();
  ...
  view(id: number): void {
    const modalRef = this.modalService.open(ViewReportComponent, { size: 'xl' });
    modalRef.componentInstance.id = id;
    modalRef.result.then(() =>
      this.thisService.fetch(this.endpoint),
      () => {}
